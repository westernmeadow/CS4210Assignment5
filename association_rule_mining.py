...
# SPECIFICATION: Assocition rule mining supermarket transactions
...
for rule in rules.iterrows():
    antecedent = ''
    for i, item in enumerate(rule[1][0]):
        antecedent += item
        if i != len(rule[1][0])-1:
            antecedent += ', '
    consequent = ''
    for i, item in enumerate(rule[1][1]):
        consequent += item
        if i != len(rule[1][1])-1:
            consequent += ', '
    print(antecedent + ' -> ' + consequent)
    print('Support: %f' % rule[1][4])
    print('Confidence: %f' % rule[1][5])
...
#-->add your python code below
    print('Prior: %f' % rule[1][3])
    print('Gain in Confidence: ' + str(100*(rule[1][5]-rule[1][3])) + '\n')
